...
func TestEcser_DescribeInstances(t *testing.T) {
	ali, _ := NewAliEcsClient(pbtenant.AliRegionId_ali_cn_hangzhou, aliTenant)
	aliFailed, _ := NewAliEcsClient(pbtenant.AliRegionId_ali_cn_hangzhou, tenanter.NewTenantWithAccessKey("", ""))

	tc, _ := NewTencentCvmClient(pbtenant.TencentRegionId_tc_ap_beijing, tcTenant)
	tcFailed, _ := NewTencentCvmClient(pbtenant.TencentRegionId_tc_ap_beijing, tenanter.NewTenantWithAccessKey("", ""))
